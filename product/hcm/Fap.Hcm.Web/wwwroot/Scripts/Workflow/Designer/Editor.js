...
Editor.prototype.readGraphState = function(node)
{
    ...
    //wyf custom 流程相关
    this.graph.wfProcessUid = node.getAttribute('wfProcessUid') || '';
    this.graph.wfDiagramUid = node.getAttribute('wfDiagramUid') || '';
    this.graph.wfDiagramName = node.getAttribute('wfDiagramName') || '';
    this.graph.wfDesc = node.getAttribute('wfDesc') || '';
    this.graph.wfResultNotice = node.getAttribute('wfResultNotice') || '1';
    this.graph.wfSuspendNotice = node.getAttribute('wfSuspendNotice') || '0';
    this.graph.wfMail = node.getAttribute('wfMail') || '1';
    this.graph.wfMessage = node.getAttribute('wfMessage') || '1';
    this.graph.frmType = node.getAttribute('frmType') || 'Internal';
    this.graph.billTable = node.getAttribute('billTable') || '';
    this.graph.billTemplate = node.getAttribute('billTemplate') || '';
...
Editor.prototype.getGraphXml = function(ignoreSelection)
{
    ...
    //wyf custom 流程相关
    node.setAttribute('wfProcessUid', this.graph.wfProcessUid || '')
    node.setAttribute('wfDiagramUid', this.graph.wfDiagramUid || '');
    node.setAttribute('wfDiagramName', this.graph.wfDiagramName || '');
    node.setAttribute('wfDesc', this.graph.wfDesc || '');
    node.setAttribute('wfResultNotice', this.graph.wfResultNotice || '0');
    node.setAttribute('wfSuspendNotice', this.graph.wfSuspendNotice ||'0');
    node.setAttribute('wfMail', this.graph.wfMail || '0');
    node.setAttribute('wfMessage', this.graph.wfMessage || '0');
    node.setAttribute('frmType', this.graph.frmType || 'Internal');
    node.setAttribute('billTable', this.graph.billTable || '');
    node.setAttribute('billTemplate', this.graph.billTemplate || '');
	return node;
};
