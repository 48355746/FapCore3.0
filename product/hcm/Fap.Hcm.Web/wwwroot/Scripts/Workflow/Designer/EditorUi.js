    ...
    if (name == null) {
        ...
        try {
            if (Editor.useLocalStorage) {
                ...
                //wyf 存储到数据库
                //校验：存在一个开始和一个结束，
                if (this.validateWrokflowDiagram() == false)
                {
                    return;
                }
                var xml = encodeURIComponent(xml);
                mxUtils.post(SAVE_URL, 'filename=' + encodeURIComponent(name) +
        ...
        if (xml.length < MAX_REQUEST_SIZE) {
            new mxXmlRequest(SAVEAS_URL, 'filename=' + encodeURIComponent(name) +
...
*校验流程图wyf
*/
EditorUi.prototype.validateWrokflowDiagram = function ()
{
    let graph = this.editor.graph;
    //debugger;
    var cells = graph.getModel().cells;
    let startCount = _.filter(_.values(cells), function (o) { return o.getAttribute('nodeType') == '0' }).length;
    if (startCount > 1) {
        layer.alert('开始节点只能存在一个，请删除多余的开始节点');
        return false;
    } else if (startCount < 1)
    {
        layer.alert('必须存在一个开始节点');
        return false;
    }
    let endCount = _.filter(_.values(cells), function (o) { return o.getAttribute('nodeType') == '99' }).length;
    if (endCount > 1) {
        layer.alert('结束节点只能存在一个，请删除多余的结束节点');
        return false;
    } else if (endCount < 1)
    {
        layer.alert('必须存在一个结束节点');
        return false;
    }
    return true;
