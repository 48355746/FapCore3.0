			...
			for(var tb in toolbar_buttons) if(toolbar_buttons.hasOwnProperty(tb)) {
				...
				switch(button.name) {
					...
					case 'viewSource':
						...
						break;
				    case 'insertText':
				        toolbar += ' <a class="btn btn-sm btn-default dropdown-toggle" data-toggle="dropdown" title="' + button.title + '"><i class="ace-icon fa fa-list-alt"></i><i class="ace-icon fa fa-angle-down icon-on-right"></i></a> ';
				        toolbar += ' <ul class="dropdown-menu dropdown-light dropdown-caret">';
				        $.each(button.values, function (i, d) {
				            toolbar += ' <li><a data-edit="insertText ${' + d.colComment + '}">' + d.colComment + '</a></li> ';
				        })
				        toolbar += ' </ul>';
				        break;
					default:
