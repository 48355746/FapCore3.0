	...
	return this.each( function() {
		...
		addXmlData = function (xml, rcnt, more, adjust) {
			...
			if(ts.p.gridview === true) {
				...
				if(ts.p.grouping) {
                    if (!locdata) {
                        self.jqGrid('groupingRender', grpdata, ts.p.colModel.length, ts.p.page, rn, rowData);
						grpdata = null;
					}
		...
		addJSONData = function(data, rcnt, more, adjust) {
			...
			if(ts.p.gridview === true ) {
				...
				if(ts.p.grouping) {
                    if (!locdata) {
                        self.jqGrid('groupingRender', grpdata, ts.p.colModel.length, ts.p.page, rn, data);
						grpdata = null;
					}
				} else if(ts.p.treeGrid === true && fpos > 0) {
					$(ts.rows[fpos]).after(rowData.join(''));
		...
		buildColItems = function (top, left, parent) {
            ...
            left = left - 20,
			str1 = '<ul id="col_menu" class="ui-search-menu  ui-col-menu modal-content" role="menu" tabindex="0" style="left:'+left+'px;top:'+top+'px;width:180px">';
			for(i=0;i<len;i++) {
		...
		buildSearchBox = function (index, top, left, parent) {
			...
			// buttons
			str1 = "<div>";
			str1 += "<div class='search_buttons'><a tabindex='0' id='bs_reset' class='fm-button " + common.button + " ui-reset btn btn-white btn-inverse btn-sm'>" + texts.Reset + "</a></div>";
			str1 += "<div class='search_buttons'><a tabindex='0' id='bs_search' class='fm-button " + common.button + " ui-search btn btn-white btn-primary  btn-sm'>" + texts.Find + "</a></div>";
			str1 += "</div>";
			elem.append(str1);
			elem = $('<li class="ui-menu-item" role="presentation"></li>').append(elem);
			left = left - 20;
			elem = $('<ul id="search_menu" class="ui-search-menu modal-content" role="menu" tabindex="0" style="left:'+left+'px;top:'+top+'px;width:190px"></ul>').append(elem);
			$(parent).append(elem);
		...
		buildColMenu = function( index, left, top ){
			...
			top=parseInt(top,10) + 35;
			var fs =  $('.ui-jqgrid-view').css('font-size') || '11px';
		    var str = '<ul id="column_menu" class="ui-search-menu modal-content column-menu dropdown-menu dropdown-menu-right dropdown-light-blue dropdown-caret dropdown-closer" role="menu" tabindex="0" style="font-size:' + fs + ';left:' + left + 'px;top:' + top + 'px;">',
			...
			texts = $.jgrid.getRegional(ts, "colmenu"),
			isgroup, isfreeze; // ???  wyf修改图标
			// sorting
			if(op.sorting) {
			    str += '<li class="ui-menu-item" role="presentation"><a class="g-menu-item" tabindex="0" role="menuitem" value="sortasc"><table class="ui-common-table"><tr><td class="menu_icon"><i class="ace-icon fa fa-arrow-up"></i></td><td class="menu_text">' + texts.sortasc + '</td></tr></table></a></li>';
			    str += '<li class="ui-menu-item" role="presentation"><a class="g-menu-item" tabindex="0" role="menuitem" value="sortdesc"><table class="ui-common-table"><tr><td class="menu_icon"><i class="ace-icon fa fa-arrow-down"></i></td><td class="menu_text">' + texts.sortdesc + '</td></tr></table></a></li>';
			}
			if(op.columns) {
				str += '<li class="ui-menu-item divider" role="separator"></li>';
				str += '<li class="ui-menu-item" role="presentation"><a class="g-menu-item" tabindex="0" role="menuitem" value="columns"><table class="ui-common-table"><tr><td class="menu_icon"><i class="ace-icon fa fa-list-alt"></i></td><td class="menu_text">' + texts.columns + '</td></tr></table></a></li>';
			}
			if(op.filtering) {
				str += '<li class="ui-menu-item divider" role="separator"></li>';
				str += '<li class="ui-menu-item" role="presentation"><a class="g-menu-item" tabindex="0" role="menuitem" value="filtering"><table class="ui-common-table"><tr><td class="menu_icon"><i class="ace-icon fa fa-filter"></i></td><td class="menu_text">' + texts.filter + ' ' + (cm.label || cm.name) + '</td></tr></table></a></li>';
			}
			if(op.grouping) {
				isgroup = $.inArray(cm.name, ts.p.groupingView.groupField);
				str += '<li class="ui-menu-item divider" role="separator"></li>';
				str += '<li class="ui-menu-item" role="presentation"><a class="g-menu-item" tabindex="0" role="menuitem" value="grouping"><table class="ui-common-table"><tr><td class="menu_icon"><i class="ace-icon fa fa-align-left"></i></td><td class="menu_text">' + (isgroup !== -1 ? texts.ungrouping : texts.grouping + ' ' + (cm.label || cm.name)) + '</td></tr></table></a></li>';
			}
			if(op.freeze) {
				isfreeze = (cm.frozen && ts.p.frozenColumns) ? false : true;
				str += '<li class="ui-menu-item divider" role="separator"></li>';
				str += '<li class="ui-menu-item" role="presentation"><a class="g-menu-item" tabindex="0" role="menuitem" value="freeze"><table class="ui-common-table"><tr><td class="menu_icon"><i class="ace-icon fa fa-text-height"></i></td><td class="menu_text">' + (isfreeze ? (texts.freeze + " " + (cm.label || cm.name)) : texts.unfreeze) + '</td></tr></table></a></li>';
			}
			str += "</ul>";
			$('body').append( str );
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			if(ts.p.colMenu && tmpcm.colmenu) {
				thead += "<a class='colmenu' href='javascript:void(0)'><span class='colmenuspan "+iconbase+' '+colmenustyle.icon_menu+"'></span></a>";
			}
			thead += "</div></th>";
		}
		thead += "</tr></thead>";
...
$.jgrid.extend({
	...
	getRowData : function( rowid, usedata ) {
		var res = {}, resall, getall=false, len, j=0,inlineValidate=true;
		this.each(function(){
            var $t = this, nm, ind;
			...
			while(j<len){
				...
				if( $(ind).hasClass('jqgrow') ) {
					if(usedata) {
                        ...
                        //wyf start增加编辑状态下获取数据
                        var editable = $(ind).attr("editable") || "0";
                        if (editable === "1" && $t.p.editurl==='clientArray') {
                            //先保存
                            var success = $($t).jqGrid("saveRow", ind.id);
                            if (!success) {
                                //校验失败
	...
	setRowData : function(rowid, data, cssp) {
		...
		this.each(function(){
			...
			if( data ) {
                try {
                    //wyf start,new rows id modify
                    if (data.Id !== undefined && rowid !== data.Id) {
                        this.p.selrow = data.Id;
                        ind.id = data.Id;
                    }
                    //wfy end
...
//Helper functions
	...
	info_dialog : function(caption, content,c_b, modalopt) {
		var mopt = {
			...
			left:250,
			top:170,
            zIndex: 1060,//wyf modify old value --zIndex : 1000,
			jqModal : true,
			modal : false,
			closeOnEscape : true,
			align: 'center',
...
// Form Functions
    ...
    var elem = "", $t = this;
        //wyf 兼容IE
    if (eltype === undefined) { eltype = 'text'; }
			...
			case "custom" :
				...
				try {
                    if ($.isFunction(options.custom_element)) {
                        //wyf添加，传入行数据rowData
                        var rowdata = $(this).jqGrid("getRowData", options.rowId);
                        var celm = options.custom_element.call($t, vl, options, rowdata);
						if(celm) {
	...
	filterToolbar : function(p){
		...
		return this.each(function(){
			...
			buildRuleMenu = function( elem, left, top ){
				...
				var fs =  $('.ui-jqgrid-view').css('font-size') || '11px';
				var str = '<ul id="sopt_menu" class="ui-search-menu modal-content" role="menu" tabindex="0" style="font-size:'+fs+';left:'+left+'px;top:'+top+'px;width:">',
	...
	searchGrid : function (p) {
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
			    ...
			    //wyf layer==========不为空，每次都重绘条件
			    if (p.layer !== null) {
			        if ($("#" + fid).get(0) !== undefined) {
			            $("#" + fid).remove();
			        }
			    }
                //========================
				...
				p.gbox = "#gbox_"+fid;
				if (p.layer) {
				    //wyf修改，===========设置layer后 查询面板不弹出
				    $("#" + $.jgrid.jqID(p.layer)).html(fil);
                    //====================================
...
$.jgrid.extend({
	...
	groupingRender : function (grdata, colspans, page, rn,data ) {
		return this.each(function(){
			var $t = this,
			grp = $t.p.groupingView,
			...
			$.each(grp.groups,function(i,n){
				...
				try {
					if ($.isArray(grp.formatDisplayField) && $.isFunction(grp.formatDisplayField[n.idx])) {
                        ...
                        gv = $t.formatter(hid, n.displayValue, cp[n.idx], data.rows[n.startRow]);//wyf注释传所有数据 n.value );
					}
				} catch (egv) {
					gv = n.displayValue;
				}
				...
				$.ajax($.extend({
					...
					complete: function(res,stat){
						...
						if (stat === "success"){
                            ...
                            if (!$.isArray(sucret)) { sucret = [true, res.responseJSON.data]; }//wyf tmp3];}
			...
			if(o.restoreAfterSelect === true || o.saveAfterSelect === true) {
				$($t).on("jqGridBeforeSelectRow.inlineNav", function( event, id ) {
					if($t.p.savedRow.length > 0 && $t.p.inlineNav===true && ( id !== $t.p.selrow && $t.p.selrow !==null) ) {
                        ...
                        //wyf 设置当前行选中
                        $($t).jqGrid('setSelection', id, false);
				});
			}
		...
		if(unformatFunc !== undefined && $.isFunction(unformatFunc) ) {
			...
			switch(formatType) {
				...
				case 'number' :
					...
					break;
			    case 'percent'://百分数自己加的 wyf
			        ret = $(cellval).text().replace('%', "");
			        break;
				case 'currency':
