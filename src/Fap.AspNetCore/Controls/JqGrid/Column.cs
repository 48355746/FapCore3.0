...
namespace Fap.AspNetCore.Controls.JqGrid
{
    public class Column
    {
        ...
        private string _unFormat;
        private string _dataInit;
        /// <summary>
        /// 格式化数据样式
        /// </summary>
        public Column SetUnFormat(string unformat)
        {
            _unFormat = unformat;
            return this;
        }
        public Column SetDataInit(string datainit)
        {
            _dataInit = datainit;
            return this;
        }
        ...
        public Column SetColmenu(bool? colmenu)
        {
            _colmenu = colmenu;
            return this;
        }
        ...
        /// 设置编辑按钮
        /// </summary>
        /// <param name="operButtons"></param>
        /// <returns></returns>
        public Column SetOperButtons(List<OperButton> operButtons)
        {
            if (_formatter.HasValue)
            {
                throw new Exception(
                    "You cannot set a formatter and a OperButtons at the same time, please choose one.");
            }
            if (!_customFormatter.IsMissing())
            {
                throw new Exception(
        ...
        /// <summary>
        /// 定义一个列是否出现在显示/隐藏 列表中（default :false）
        /// </summary>
        /// <param name="hidden"></param>
        /// <returns></returns>
        public Column SetHidelg(bool hidden)
        {
            _hidedlg = hidden;
            return this;
        }
        ...
        {
            get { return !_defaultSearchValue.IsMissing(); }
        }
        ...
        public override string ToString()
        {
            ...
            //hidedlg 
            if (_hidedlg.HasValue) script.AppendFormat("hidedlg:{0},", _hidedlg.Value.ToString().ToLower()).AppendLine();
            // Key
            if (_key.HasValue) script.AppendFormat("key:{0},", _key.Value.ToString().ToLower()).AppendLine();

            // Label
            if (!_label.IsMissing()) script.AppendFormat("label:'{0}',", _label).AppendLine();
            ...
            if (_colmenu.HasValue) script.AppendFormat("colmenu:{0},", _colmenu.Value.ToString().ToLower()).AppendLine();
            // Editable
